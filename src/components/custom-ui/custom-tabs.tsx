...
const useCustomTabsContext = () => {
  ...
  if (!context) {
    throw new Error("CustomTabs components must be used within a CustomTabs component");
  }
  return context;
};
...
const CustomTabs = ({ value, onValueChange, children, className }: CustomTabsProps) => {
  return (
    <CustomTabsContext.Provider value={{ activeTab: value, setActive: onValueChange }}>
...
const CustomTabsTrigger = ({ value, children, className, icon }: CustomTabsTriggerProps) => {
  ...
  return (
    <button
      ...
      className={cn(
        "text-foreground-secondary flex items-center justify-center gap-1.5 border-b-2 border-transparent p-2 text-sm font-medium whitespace-nowrap lg:text-base",
...
const CustomTabsContent = ({ value, children, className }: CustomTabsContentProps) => {
