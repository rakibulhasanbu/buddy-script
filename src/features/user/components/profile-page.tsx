...
export const ProfilePage = ({ user, isLoading, isError, isOwner }: ProfilePageProps) => {
  ...
  if (isError || !user) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="rounded-md bg-white p-8 text-center shadow-[0_4px_8px_rgba(0,0,0,0.08)]">
          <h2 className="mb-2 text-xl font-medium text-[#212121]">Something went wrong</h2>
          <p className="text-[#666666]">We could not load this profile. Please try again later.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          ...
          <div className="space-y-4">
            <div className="rounded-md bg-white p-5 shadow-[0_4px_8px_rgba(0,0,0,0.08)]">
              <h3 className="mb-4 text-lg font-medium text-[#212121]">Intro</h3>
              {user.bio ? (
                <p className="text-[#666666]">{user.bio}</p>
              ) : (
                <p className="text-sm text-[#999999]">No bio added yet.</p>
              )}
            </div>
          </div>
          ...
          {/* Posts */}
          <div>
            <div className="mb-4 rounded-md bg-white p-4 shadow-[0_4px_8px_rgba(0,0,0,0.08)]">
              <h3 className="text-lg font-medium text-[#212121]">Posts</h3>
            </div>
